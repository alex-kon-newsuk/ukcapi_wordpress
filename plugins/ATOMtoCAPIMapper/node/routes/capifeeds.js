...
{
  request({
        url: 'http://cdn.newsapi.com.au/dev-int/content/v2/',
        qs:
        {
          api_key: '87865c9kf5wncy5vappm4tjv',
          query: 'categoryPaths.path:"/display/thesun.co.uk/news/testing/"'
          //query: 'categoryPaths.path:"' + req.query.name + '"'
        },
        method: 'GET'
        },
        function (error, response, body)
        {
          console.log('Calling AU-CAPI document category API: ' + response.statusCode);
          var stories = JSON.parse(response.body);

          res.setHeader('Content-Type', 'text/html');
          res.write('<rss>');
          res.write('<channel>');
          res.write('<title>CAPI Feeds</title>');
          res.write('<count>' + stories.totalHits + '</count>');
          res.write('</channel>');

          _.each(stories.results, function(item)
          {
            res.write('<item>');
            res.write('<guid>' + item.id.value + '</guid>');
            res.write('<link rel="absolute">' + item.id.link + '</link>');
            res.write('<title>' + item.title + '</title>');
            res.write('<summary>' + item.standFirst + '</summary>');
            res.write('<description>' + item.description + '</description>');
            res.write('<author>' + item.byline + '</author>');
            res.write('<pubDate>' + item.dateLive + '</pubDate>');
            res.write('<thumbnail url="" />');
            res.write('<style>' + item.contentType + '</style>');
            res.write('<data><![CDATA[' + JSON.stringify(item) + ']]></data>');
            res.write('</item>');
          });
          res.write('</rss>')
          return res.status(200).end();
